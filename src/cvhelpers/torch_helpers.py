...
class CheckPointManager(object):
    ...
    def _save_checkpoint(self, step, model, score, **kwargs):
        ...
        if matching_files:
            # 从匹配的文件名中提取得分
            existing_score = float(re.search('-([0-9.]+).pth', matching_files[0]).group(1))
            
            # 如果当前得分不比已存在的得分高，则不保存
            if score <= existing_score:
                self._logger.info('Existing checkpoint has better or equal score. Skipping save.')
                return
